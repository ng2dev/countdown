...
func GenerateApp(options *server.Options) (abci.Application, error) {
	...
	var dbPath string
	if options.Home != "" {
		dbPath = filepath.Join(options.Home, "countdown.db")
	}

	stack := Stack(nil, options.MinFee)
	application, err := Application("countdown", stack, TxDecoder, dbPath, options.Debug)
	if err != nil {
		return nil, err
	}
...
func InlineApp(kv weave.CommitKVStore, logger log.Logger, debug bool) abci.Application {
	...
	store := app.NewStoreApp("countdown", kv, QueryRouter(), ctx)
