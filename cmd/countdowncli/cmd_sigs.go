...
`)
	...
	var (
		tmAddrFl = fl.String("tm", env("COUNTDOWNCLI_TM_ADDR", "https://COUNTDOWN.NETWORK.iov.one:443"),
			"Tendermint node address. Use proper NETWORK name. You can use COUNTDOWNCLI_TM_ADDR environment variable to set it.")
		keyPathFl = fl.String("key", env("COUNTDOWNCLI_PRIV_KEY", os.Getenv("HOME")+"/.countdown.priv.key"),
			"Path to the private key file that transaction should be signed with. You can use COUNTDOWNCLI_PRIV_KEY environment variable to set it.")
	)
	fl.Parse(args)
	...
	cl := client.NewClient(client.NewHTTPConnection(*tmAddrFl))
	seq, err := cl.NextNonce(key.PublicKey().Address())
	if err != nil {
