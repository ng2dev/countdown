...
func TestCmdSendTokensHappyPath(t *testing.T) {
	...
	assert.Equal(t, coin.NewCoinp(5, 0, "COUNTDOWN"), msg.Amount)
}
...
func TestCmdWithFeeHappyPath(t *testing.T) {
	sendMsg := &cash.SendMsg{
		...
		Amount:      coin.NewCoinp(5, 0, "COUNTDOWN"),
		Memo:        "a memo",
	}
	sendTx := &countdown.Tx{
	...
	assert.Equal(t, coin.NewCoinp(5, 0, "COUNTDOWN"), tx.Fees.Fees)
...
func TestCmdWithFeeHappyPathDefaultAmount(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"only minimal fee": {
			Conf: cash.Configuration{
				Metadata:   &weave.Metadata{Schema: 1},
				MinimalFee: coin.NewCoin(4, 0, "COUNTDOWN"),
			},
			Fees:    nil,
			WantFee: coin.NewCoinp(4, 0, "COUNTDOWN"),
		},
		"only message fee": {
			Conf: cash.Configuration{
				Metadata:   &weave.Metadata{Schema: 1},
			...
			Fees: map[string]coin.Coin{
				sendMsg.Path(): coin.NewCoin(17, 0, "COUNTDOWN"),
			},
			WantFee: coin.NewCoinp(17, 0, "COUNTDOWN"),
		},
		"custom message fee is more important than global setting": {
			Conf: cash.Configuration{
				Metadata:   &weave.Metadata{Schema: 1},
				MinimalFee: coin.NewCoin(123, 0, "COUNTDOWN"),
			},
			Fees: map[string]coin.Coin{
				sendMsg.Path(): coin.NewCoin(11, 0, "COUNTDOWN"),
			},
			WantFee: coin.NewCoinp(11, 0, "COUNTDOWN"),
		},
	}
